import { isIn, isInt, isLength } from 'validator';
...
export type CharsetDirection = 'source' | 'target';
...
export interface Validation {
  readonly data?: GetQRData;
  readonly errors?: ReadonlyArray<InvalidField>;
}
...
export default function validateQrData(data: GetQRData): Validation {
  ...
  const fieldValidations: ReadonlyArray<FieldValidation> = [
    ...
    },
    {
      ...
      message: getCharsetMessage('source'),
      result: data.charsetSource
        ? isIn(data.charsetSource, charsetTypes as any)
        : true
    },
    {
      fieldName: 'charsetTarget',
      message: getCharsetMessage('target'),
      result: data.charsetTarget
        ? isIn(data.charsetTarget, charsetTypes as any)
        : true
    },
    {
      fieldName: 'ecc',
    ...
    },
    {
      fieldName: 'color',
      message:
        'color should be of the following formats ([0-255]-[0-255]-[0-255], [a-ab-a], [aabbcc], [abc])',
      result: data.color ? checkColor(data.color) : true
    },
    {
      fieldName: 'bgColor',
      message:
        'bgColor should be of the following formats ([0-255]-[0-255]-[0-255], #[aabbcc], #[abc])',
    ...
    },
    {
      ...
      result: data.margin
        ? isInt(data.margin.toString(), { min: 0, max: 50 })
        : true
    },
    {
      fieldName: 'qZone',
      ...
      result: data.qZone
        ? isInt(data.qZone.toString(), { min: 0, max: 100 })
        : true
    }
  ];
  ...
  return {
    data,
    errors: errorFields.length ? getInvalidFields(errorFields) : []
  };
}

export function getCharsetMessage(direction: CharsetDirection): string {
  return `charset ${direction} should be one of (${charsetTypes.join(', ')})`;
}

export function getEccMessage(): string {
  return `ecc should be one of (${eccTypes.join(', ')})`;
}

export function maxSizeForFormat(format: Format = DEFAULT_FORMAT): number {
  return isIn(format, ['png', 'gif', 'jpeg']) ? 1000 : 1000000;
}

function checkMax(value: number, maxValue: number): boolean {
